	...
	return this.each( function() {
		...
		updatepager = function(rn, dnd) {
			var cp, last, base, from,to,tot,fmt, pgboxes = "", sppg,
                        tspgId = ts.p.pager ? $.jgrid.jqID(ts.p.pager.substr(1)) : "",
			...
			if(pgboxes) {
				...
				if (ts.p.viewrecords){
					if(ts.p.reccount === 0) {
                                                $('#' + tspgId+ '_left').hide();
                                                $('#' + tspgId+ '_center').hide();
                                                 $('#pg' + tspg).attr("style","background-color: white;");
//                                               
                                                $(".ui-paging-info",pgboxes).attr("style","text-align: center;");
						$(".ui-paging-info",pgboxes).html(ts.p.emptyrecords);
					} else {  $('#' + tspgId+ '_left').show();  
                                                 $('#' + tspgId+ '_center').show();
                                                 $('#pg' + tspg).removeAttr("style")
                                                  $(".ui-paging-info",pgboxes).attr("style","text-align: right;");
						from = base+1;
						tot=ts.p.records;
						if($.fmatter) {
		...
		setPager = function (pgid, tp){
			...
			$('input.ui-pg-input',"#"+pgcnt).keypress( function(e) {
				...
				if(ts.p.toppagerOnly){
		   	if(pgid.indexOf("toppager")!=-1){
			    //$('#' + pgid+ '_left').remove();
				//
				$('#' + pgid + '_left').html($('#' + pgid + '_right').html());
				$('#' + pgid + '_left').attr("align","left");
                               
				$('#' + pgid + '_left div').attr("style","text-align: left;");
				$('#' + pgid + '_right').remove();
				$('#' +pgid+ '_center').attr("align","right");
				$('#' + pgid+ '_center').attr("colspan","2");
